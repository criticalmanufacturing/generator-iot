...
export class fontGen {
    ...
    public async go(sourceDir: string, targetDir: string): Promise<void> {
        return (new Promise<void>((resolve, reject) => {
            ...
            this.log(chalk.gray("Reading configuration and generating metadata..."));
            const map = this.generateMap(sourceDir, targetDir);

            this.log(chalk.yellow("Glyphs found:"));
            for (let glyph of map.glyphs) {
                this.log(chalk.blue(`   ${glyph.file}`));
            }

            this.log(chalk.gray(`Found '${map.glyphs.length}' files, starting generation...`));
            ...
            webfontsGenerator({
                ...
                if (error) {
                    ...
                    self.log("");
                    self.log(chalk.yellow("Available icons to use with tasks:"));
                    for (let glyph of map.glyphs) {
                        if (glyph.cssClass.indexOf("-lg-") !== -1) {
                            self.log(chalk.blue(`   ${glyph.cssClass}`));
                        }
                    }
