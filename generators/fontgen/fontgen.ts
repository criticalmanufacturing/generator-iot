...
export class fontGen {
    ...
    private generateMap(sourceDir: string, targetDir: string): any {
        const map = Object.assign({}, this.map_default);

        map.sourceDir = sourceDir;
        map.targetDir = targetDir;

        // 1. Read configuration file to get specific font options
        const fcPath = path.join(sourceDir, this.fontConfig);

        if (fs.existsSync(fcPath)) {
            // Require file
            // eslint-disable-next-line @typescript-eslint/no-var-requires
            const fontCfg = require(path.resolve(".", fcPath));
            // Merge configs
    ...
    private writeIndex(map: any) {
        ...
        const finalTsFileContent = "export default " + JSON.stringify(indexStructure);
        const indexPath = path.resolve(path.join(map.targetDir, "metadata.ts"));
    ...
    private writeLESS(map: any) {
        ...
        const abstractCssIds = ["/* abstract CSS selectors to be re-used */"];
        const specCssIds = ["", "/* default CSS selectors available for direct use in the recommended cases */"];

        for (const glyph of map.glyphs) {
            const json = JSON.stringify(glyph.code).replace(/[\u0080-\uFFFF]/g, function (m) {
                return "\\" + ("0000" + m.charCodeAt(0).toString(16)).slice(-4);
            });
            const cssId = `.abstract-${glyph.cssClass} { content: ${json}; } /* '${glyph.code}' */`;
            abstractCssIds.push(cssId);

            const specId = `.${glyph.cssClass}:before { .abstract-${glyph.cssClass} } /* '${glyph.code}' */`;
        ...
        const lessConfigFile = `${map.font.prefix}-config.less`;
        const lessConfig = path.resolve(path.join(map.targetDir, lessConfigFile));
        ...
        const lessCSS = path.resolve(path.join(map.targetDir, `${map.font.prefix}-font.less`));
    ...
    private writeCSS(map: any) {
        ...
        for (const glyph of map.glyphs) {
            ...
            const cssId = `.${glyph.cssClass}:before { content: ${json}; } /* '${glyph.code}' */`;
            cssIds.push(cssId);
        }
        ...
        const css = path.resolve(path.join(map.targetDir, `${map.font.fontname}.css`));
